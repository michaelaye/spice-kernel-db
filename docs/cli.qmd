...
spice-kernel-db acquire juice_ops.tm

# By filename with explicit mission (when multiple missions are known)
spice-kernel-db acquire juice_ops.tm --mission JUICE

# By full URL
spice-kernel-db acquire https://naif.jpl.nasa.gov/pub/naif/JUICE/kernels/mk/juice_ops.tm

# Auto-confirm download
spice-kernel-db acquire juice_ops.tm -y
```

When a filename is given instead of a URL, the tool looks up the mk/ directory URL from previously acquired metakernels. If multiple missions are known, use `--mission` to disambiguate.
...
```bash
# List all known mk/ directories (derived from previously acquired metakernels)
spice-kernel-db browse

# Browse by mission name (looks up the URL from previous acquisitions)
...
**Without a URL**, lists all remote `mk/` directories known from previous `acquire` commands, along with how many metakernels have been acquired from each:
