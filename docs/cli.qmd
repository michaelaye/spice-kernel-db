...
| Option | Description |
|--------|-------------|
| `name_or_hash` | A kernel filename (canonical *or* alias) or a SHA-256 (full or ≥6-char prefix) |

```bash
spice-kernel-db aliases bc_mpo_hga_scm_20220101_20230101_s20221229_v01.bc
```

Prints the content hash, canonical name, kernel type and size, the full list of alias filenames (with the canonical one marked), and every on-disk location. Exits non-zero if the name or hash is unknown.
