...
Remove stale database entries. Two modes:
...
| `--metakernels`, `--mk` | Switch to metakernel-registry mode. Sends a HEAD to each row's effective URL (`source_url` if set, otherwise `mission.mk_dir_url + filename`) and treats 403/404/410 as permanently dead. |
| `--delete-files` | With `--metakernels --execute`: also unlink the on-disk `.tm` files. |

Network errors (timeouts, DNS failures) are **never** treated as dead — pruning would otherwise discard real state when you happened to be offline.

```bash
# Default mode — preview stale kernel locations
spice-kernel-db prune

# Actually remove
spice-kernel-db prune --execute

# Find dead metakernel registry rows (NAIF rotates old snapshots into former_versions/)
spice-kernel-db prune --metakernels

# Remove the registry rows
spice-kernel-db prune --metakernels --execute

# And delete the local .tm files (symlink trees are shared and never auto-deleted)
spice-kernel-db prune --metakernels --execute --delete-files
```
