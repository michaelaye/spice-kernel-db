...
```python
db.list_known_mk_dirs() -> list[dict]
```

List known remote `mk/` directory URLs, derived from previously acquired metakernels in the `metakernel_registry`. Each entry shows the mission name, directory URL, and how many metakernels have been acquired from it.

Returns a list of dicts with keys: `mission`, `mk_dir_url`, `n_acquired`.
