...
List all tracked metakernels, optionally filtered by mission. Each entry includes the number of kernel entries and how many are available in the database.

Prints a summary table and returns a list of dicts with keys: `filename`, `mission`, `source_url`, `acquired_at`, `mk_path`, `n_kernels`, `n_available`.
