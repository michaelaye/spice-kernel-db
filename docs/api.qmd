...
Every distinct filename a content hash is registered under is recorded in the `kernel_aliases` table (`(sha256, filename, first_seen, source_url)`, PK `(sha256, filename)`), populated by `register_file` on each registration — including the as-referenced name when a dedup symlink is registered, so the trail is never lost. The companion `alias_counts(shas: list[str]) -> dict[str, int]` maps each hash to its number of known names.
