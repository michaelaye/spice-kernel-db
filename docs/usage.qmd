...
```
Remote metakernels: JUICE (https://spiftp.esac.esa.int/data/SPICE/JUICE/kernels/mk/)

  Metakernel              Versions  Latest           Local
  ──────────────────────  ────────  ──────────────── ────────
  juice_crema_5_1.tm            12  2024-11-27 09:30 yes
  juice_ops.tm                   8  2024-12-03 14:00 outdated
  juice_sci.tm                   5  2024-10-15 11:00 no
  juice_struct.tm                1  2024-01-10 08:00 no

  Total: 4 unique | 26 files | 1 locally acquired
...
- **Local**: `yes` (up to date), `outdated` (remote copy is newer), or `no` (not acquired)
...
## Listing kernels in a metakernel

The `list` command shows all kernels inside a metakernel, grouped by type:

```bash
spice-kernel-db list juice_ops.tm
```

```
Metakernel: juice_ops.tm
Kernels: 42 total
(5 ck, 2 fk, 1 ik, 1 lsk, 3 pck, 1 sclk, 29 spk)
...
```bash
spice-kernel-db list juice_ops.tm --type spk
```

Without arguments, you'll be prompted to pick from locally acquired metakernels.
